...
def main():
    ...
    if scelta_media == "Dati medi":
        ...
        res_vis = df_results.loc[(df_results.Season == scelta_season) & (df_results.my_team == scelta_team)][["Squadra", "Data", "Luogo","Chiav", "Avversari", "W/L"]].reset_index(drop=True).sort_values(by=['Data'], ascending=False)
        st.markdown("### Risultati:")
        st.write(res_vis)
        st.markdown("### Statistiche giocatori:")
